...
install_dependencies() {
    ...
    case "${installed_pkg_mgr}" in
        "apt")
            ...
            BUILDDEPS="${INSTALL_CMD} cmake python3 python3-dev gcc g++ build-essential libgcr-3-dev libwebkit2gtk-4.0-dev"
            ;;
        "yum")
            INSTALL_CMD="${SU_CMD} ${installed_pkg_mgr} -y update && ${SU_CMD} ${installed_pkg_mgr} install"
            BUILDDEPS="${INSTALL_CMD} python3{,-devel,-pip} cmake gcc{,-c++} kernel-devel libgcr-3 webkit2gtk"
            ;;
        "pacman")
            INSTALL_CMD="${SU_CMD} ${installed_pkg_mgr} -Syyy && ${SU_CMD} ${installed_pkg_mgr} -S"
            BUILDDEPS="${INSTALL_CMD} base-devel gcc cmake gcr python{,-pip} webkit2gtk"
            ;;
        "pkg_add")
            INSTALL_CMD="${SU_CMD} syspatch && ${SU_CMD} ${installed_pkg_mgr} -u && ${SU_CMD} ${installed_pkg_mgr}"
            BUILDDEPS="${INSTALL_CMD} vim python3 py-{py,wheel,pip,setuptools} llvm cmake clang-tools-extra moreutils coreutils libgcr-3 webkit2gtk"
            ;;
        "pkg")
            INSTALL_CMD="${SU_CMD} syspatch && ${SU_CMD} ${installed_pkg_mgr} -u && ${SU_CMD} ${installed_pkg_mgr}"
            BUILDDEPS="${INSTALL_CMD} vim python3 py-{py,wheel,pip,setuptools} llvm git cmake clang-tools-extra moreutils coreutils libgcr-3 webkit2gtk"
            ;;
        *)
